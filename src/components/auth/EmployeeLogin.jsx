...
// steps: 'select' | 'otp_sent' | 'set_password' | 'enter_password' | 'forgot_otp' | 'reset_password'
...
export function EmployeeLogin() {
  ...
  const [otpIsOwner, setOtpIsOwner]   = useState(false) // true = OTP went to owner's email
  ...
  /* ── التالي بعد اختيار الموظف ── */
  const handleNext = async () => {
    if (!selectedId) return setError('اختر اسمك من القائمة')
    setError(''); setPass(''); setConf('')

    if (emp.email) {
      // موظف له إيميل → نرسل OTP
      setSubmitting(true)
      const err = await sendOtp(emp.email)
      setSubmitting(false)
      if (err) return setError('فشل إرسال رمز التحقق: ' + err.message)
      setOtpEmail(emp.email); setOtpCode(''); setStep('otp_sent')
    } else {
      // بدون إيميل → كلمة مرور
      setStep(emp.password ? 'enter_password' : 'set_password')
    }
  }

  /* ── تحقق من OTP لتسجيل الدخول ── */
  ...
  const handleForgotPassword = async () => {
    ...
    setSubmitting(false)
    if (err) return setError('فشل إرسال الرمز: ' + err.message)
    setPendingEmp(emp); setOtpEmail(email); setOtpIsOwner(isOwner); setOtpCode(''); setError('')
    setStep('forgot_otp')
  }

  /* ── تحقق من OTP لإعادة التعيين ── */
  ...
  const handleResetPassword = async () => {
    ...
    setSubmitting(true)
    const { data, error } = await supabase.from('employees')
      .update({ password: btoa(password) }).eq('id', pendingEmp.id).select().single()
    if (error) { setSubmitting(false); return setError('حدث خطأ، حاول مجدداً') }
    await employeeLogin(data)
    setSubmitting(false)
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-5">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-8 space-y-5">
          ...
          {/* ── OTP تسجيل الدخول (موظف له إيميل) ── */}
          {step === 'otp_sent' && (
            <>
              <div className="text-center space-y-2">
                <div className="w-14 h-14 bg-blue-100 dark:bg-blue-900/40 rounded-2xl flex items-center justify-center mx-auto">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">تم إرسال رمز التحقق لبريدك</p>
                <p className="text-sm font-medium text-blue-600 dark:text-blue-400" dir="ltr">{maskEmail(otpEmail)}</p>
              </div>
              ...
              <button onClick={handleOtpLogin} disabled={submitting}
                className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-medium rounded-xl transition-colors">
              ...
              <button onClick={goBack} className="w-full py-1.5 text-sm text-gray-400 dark:text-gray-500 hover:underline">رجوع</button>
            </>
          )}

          {/* ── تعيين كلمة مرور (أول مرة — بدون إيميل) ── */}
          {step === 'set_password' && (
            <>
              <div className="text-center space-y-2">
